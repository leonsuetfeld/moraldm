...
load("C:/Users/behinger/cloud/PhD/Supervision/Leon/stan_model_1_withPriors.RData")
...
# BayesFactors
hypothesis(mres_full,paste0(m1$effect,'=0'))
hypothesis(mres_agesexsdsvg,paste0(m2$effect,'=0'))

m = rbind(m1,m2) 


label_effects = function(x){
  # Function to classify effects in different ways (maineffect, 2x, 3x interactions), so that we can separately plot them later  
  # Rnames sex differences to gender differences
...
ranef_to_df = function (mres){
  #extract the random effects from the brm fit and labels them to be usable with the label_effect function 
  r = ranef(mres)$sn_idx
...
# label the random & fixed effects
r = rbind(r1,r2)
m = m%>%label_effects
r = r%>%label_effects
...
plot_names = function(x,short=FALSE){
  #rename the plots with something nicer
  rename_frame = list(
